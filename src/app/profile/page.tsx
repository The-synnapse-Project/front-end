      ...
      if (session?.user?.id) {
        try {
          ...
          if (apiUser) {
            ...
            // Check if user is a Google user using the googleId field from the database
            // or fall back to checking if they have a Google profile image
            setIsGoogleUser(!!personData.googleId || !!session.user.image);
          }
        } catch (error) {
